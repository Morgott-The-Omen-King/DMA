...
#         #     # First level: layer3 -> layer2
#         #     feat_1_16_to_1_8 = F.interpolate(
#         #         self.layer3_conv(feat_1_16),
#         #         size=feat_1_8.shape[-2:],
#         #         mode='bilinear',
#         #         align_corners=False
#         #     )
#         #     feat_1_8_fused = self.layer2_conv(feat_1_8) + feat_1_16_to_1_8

#         #     # Second level: fused layer2 -> layer1 
#         #     feat_1_8_to_1_4 = F.interpolate(
#         #         feat_1_8_fused,
...
#         #     # Final fusion with layer1 features
#         #     mask_feats = self.fusion_conv(feat_1_4 + feat_1_8_to_1_4)
...
class MaskFormerDecoder(nn.Module):
    ...
    #     decoder_output = self.decoder_norm(output)
    #     decoder_output = decoder_output.transpose(0, 1)
    #     mask_embed = self.mask_embed(decoder_output)  # [B, Q, C*9]
        
    #     # Reshape mask_embed into 3x3 convolution kernels
    #     B, Q, C9 = mask_embed.shape
    #     C = C9 // 9
    #     mask_embed = mask_embed.view(B, Q, C, 3, 3)  # [B, Q, C, 3, 3]
    #     H, W = mask_features.shape[-2:]
    #     # Apply convolution using the reshaped kernels
    #     mask_features = mask_features.view(B, -1, C, H, W)  # [B, T, C, H, W]
    #     outputs_mask = []
        
    #     for i in range(Q):
    #         # Extract kernel for current query
    #         kernel = mask_embed[:, i]  # [B, C, 3, 3]
            
    #         # Process each batch separately
    #         batch_masks = []
    #         for b in range(B):
    #             # Get kernel and features for current batch
    #             curr_kernel = kernel[b]  # [C, 3, 3] 
    #             curr_features = mask_features[b]  # [T, C, H, W]
                
    #             # Apply convolution
    #             curr_mask = F.conv2d(curr_features,
    #                                curr_kernel.unsqueeze(0),  # [1, C, 3, 3]
    #                                padding=1,
    #                                stride=1)  # [T, 1, H, W]
                
    #             batch_masks.append(curr_mask.squeeze(1))  # [T, H, W]
                
    #         # Stack batch results
    #         curr_mask = torch.stack(batch_masks)  # [B, T, H, W]
    #         outputs_mask.append(curr_mask)
            
    #     outputs_mask = torch.stack(outputs_mask, dim=1)  # [B, Q, T, H, W]
    #     b, q, t, _, _ = outputs_mask.shape
    ...
    def forward(self, img_feats, query_feats, query_pos_embed, query_ms_feats):
        ...
        # masks = masks.sum(dim=2)  # (B, K, T, H, W)
        # masks = masks.clamp(0, 1)  # 将值截断到0和1之间
        masks = outputs_mask[:, 0].view(B, K, T, H, W)

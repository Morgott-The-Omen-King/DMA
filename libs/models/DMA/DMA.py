...
class DMA(nn.Module):
    ...
    def __init__(self, 
        ...
        # ========================================================
        # Initialize weights
        self.init_weights()  # 加上这个部分就会覆盖R50部分预训练的权重
...
class MaskFormerDecoder(nn.Module):
    ...
    def forward(self, img_feats, query_feats, query_pos_embed, query_ms_feats):
        ...
        for i in range(self.num_feature_levels):
            ...
            _, c, hw = src[-1].shape
            pos[-1] = pos[-1].unsqueeze(1).repeat(1, K, 1, 1, 1).view(B*K, T, c, hw).permute(1, 3, 0, 2).flatten(0, 1)  # 复制了k-shot维度
            src[-1] = src[-1].unsqueeze(1).repeat(1, K, 1, 1, 1).view(B*K, T, c, hw).permute(1, 3, 0, 2).flatten(0, 1)
        
        # 通过transformer layers
        output = query_feats  # (N, B*K, C)
        
        mask_features = mask_features.unsqueeze(1).repeat(1, K, 1, 1, 1, 1).flatten(0, 1)

...
class YTVOSDataset(Dataset):
    def __init__(self, data_path=None, train=True, valid=False,
        ...
        self.ann_file = os.path.join(data_dir, 'train.json')
        ...
        if not self.train:
            ...
            for i in range(len(self.class_list)):
                for j in range(len(self.video_ids[i]) - support_frame):  # remove the support set  # 每一个类别中去掉5个视频
    ...
    def __gettrainitem__(self, idx):
        ...
        support_vid = random.sample(vid_set, self.support_frame)  # 选5个视频
